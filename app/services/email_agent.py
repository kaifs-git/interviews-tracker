...
TOOLS = [
    {
        ...
        "description": "Create a new job application record when email shows a new application was submitted or a recruiter reached out about a specific role.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "notes": {"type": "string", "description": "Any relevant notes from the email"},
    ...
    },
    {
        ...
        "description": "Update the stage or final result of an existing application. Use when email is a rejection, offer, shortlisting, or stage update.",
        "input_schema": {
            "type": "object",
            "properties": {
                "company_name": {"type": "string", "description": "Company name to find the application"},
                "job_title": {"type": "string", "description": "Job title hint to identify correct application"},
    ...
    },
    {
        ...
        "description": "Create an interview round when email schedules or confirms an interview. Extract date/time/link carefully.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "meeting_link": {"type": "string", "description": "Zoom/Meet/Teams link if in email"},
    ...
    {
        "name": "create_contact",
        "description": "Save a recruiter or HR contact from their email. Use when you can extract a real person's name and role.",
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string"},
    ...
    {
        "name": "skip_email",
        "description": "Skip this email — not related to job search at all (newsletters, OTPs, promotions, etc.)",
        "input_schema": {
            "type": "object",
            "properties": {
                "reason": {"type": "string"}
    ...
    },
    {
        ...
        "description": "Flag for manual review when you're unsure what to do or the email is ambiguous.",
        "input_schema": {
            "type": "object",
            "properties": {
                "reason": {"type": "string"},
...
Given an email, identify if it's related to job search and take the appropriate action using the provided tools.

Rules:
- One email can trigger multiple tool calls (e.g., create_contact AND update_application_status)
- For rejection emails: call update_application_status with final_result="Rejected" and new_stage="Rejected"
- For offer emails: call update_application_status with new_stage="Offer" and final_result="Selected"
- For interview scheduling: call create_interview_round AND optionally create_contact for the recruiter
- For initial outreach/application confirmation: call create_application
- If you cannot find an existing application for a status update, create a new one first
- Always extract contact info when a real person emails (not a no-reply)
- Skip newsletters, OTPs, promotional emails, system notifications
- When unsure, flag_for_review rather than guess
...
def process_email_for_user(
    ...
    """Process a single email through the Claude agent."""
    # Dedup check
    existing = db.query(AgentActivityLog).filter_by(
...
{email.get('body', '')[:3000]}"""
    ...
    for block in response.content:
        ...
        try:
            if tool_name == "create_application":
                entity_id, summary = _create_application(db, user_id, args, email)
                log.entity_type = "application"
                log.entity_id = entity_id
                log.summary = summary

            elif tool_name == "update_application_status":
                entity_id, summary = _update_application(db, user_id, args)
                log.entity_type = "application"
                log.entity_id = entity_id
                log.summary = summary

            elif tool_name == "create_interview_round":
                entity_id, summary = _create_interview(db, user_id, args, email)
                log.entity_type = "interview"
                log.entity_id = entity_id
                log.summary = summary

            elif tool_name == "create_contact":
                entity_id, summary = _create_contact(db, user_id, args)
    ...
    # Send push notification for meaningful actions
    actionable = [l for l in logs if l.action_type not in ("skipped",)]
    if actionable:
        first = actionable[0]
        notif_map = {
            ...
            "flagged": "Email Needs Review",
        }
        title = notif_map.get(first.action_type, "Agent Activity")
        body = first.summary or email.get("subject", "")
        send_push(db, user_id, title, body)
...
def _update_application(db: Session, user_id: int, args: dict):
    ...
    app = None
    if job_title_hint and apps:
        for a in apps:
            if job_title_hint.lower() in (a.job_title or "").lower():
                app = a
                break
    if not app and apps:
        app = apps[0]
