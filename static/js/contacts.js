...
const contactsPage = (() => {
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-3 flex-1 min-w-0">
            <div class="w-11 h-11 bg-gradient-to-br from-indigo-100 to-violet-100 rounded-full flex items-center justify-center text-base font-bold text-indigo-600 flex-shrink-0">
          ...
          <div class="flex gap-0.5 ml-2 flex-shrink-0">
            <button onclick="contactsPage.openEditModal(${c.id})" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><i class="fa-solid fa-pen-to-square text-xs"></i></button>
            <button onclick="contactsPage.confirmDelete(${c.id},'${c.name.replace(/'/g,"\\'")}')\" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg"><i class="fa-solid fa-trash text-xs"></i></button>
          </div>
        </div>
        ${c.email||c.phone||c.linkedin ? `
        <div class="mt-3 pt-3 border-t border-slate-50 flex flex-wrap gap-2">
          ${c.email ? `<a href="mailto:${c.email}" class="flex items-center gap-1.5 text-xs text-slate-600 bg-slate-50 hover:bg-indigo-50 px-2.5 py-1.5 rounded-lg"><i class="fa-solid fa-envelope text-slate-400 text-xs"></i>${c.email}</a>`:''}
          ${c.phone ? `<a href="tel:${c.phone}" class="flex items-center gap-1.5 text-xs text-slate-600 bg-slate-50 hover:bg-indigo-50 px-2.5 py-1.5 rounded-lg"><i class="fa-solid fa-phone text-slate-400 text-xs"></i>${c.phone}</a>`:''}
          ${c.linkedin ? `<a href="${c.linkedin}" target="_blank" class="flex items-center gap-1.5 text-xs text-blue-600 bg-blue-50 px-2.5 py-1.5 rounded-lg"><i class="fa-brands fa-linkedin text-xs"></i>LinkedIn</a>`:''}
        </div>` : ''}
  ...
  function renderDesktopTable() {
      ...
      <tr>
        <td class="px-5 py-4">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 bg-gradient-to-br from-indigo-100 to-violet-100 rounded-full flex items-center justify-center text-sm font-bold text-indigo-600 flex-shrink-0">${(c.name||'?')[0].toUpperCase()}</div>
            <div>
              <p class="font-semibold text-slate-800 text-sm">${c.name}</p>
        ...
        <td class="px-5 py-4">
          <div class="flex gap-1">
            <button onclick="contactsPage.openEditModal(${c.id})" class="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md"><i class="fa-solid fa-pen-to-square text-xs"></i></button>
            <button onclick="contactsPage.confirmDelete(${c.id},'${c.name.replace(/'/g,"\\'")}')\" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-md"><i class="fa-solid fa-trash text-xs"></i></button>
          </div>
        </td>
      </tr>
    `).join('');
    return `
      <div class="hidden sm:block bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-5 py-3.5 border-b border-slate-100"><p class="text-sm text-slate-500">${contacts.length} contact${contacts.length!==1?'s':''}</p></div>
        <div class="overflow-x-auto">
          <table class="data-table">
            <thead><tr><th>Name</th><th>Role</th><th>Company</th><th>Contact Info</th><th>Notes</th><th></th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
      </div>`;
  ...
  function renderList() {
    if (!contacts.length) {
      return emptyState('fa-address-book','No contacts yet','Add recruiters, hiring managers, and other contacts.',btn('Add Contact','contactsPage.openAddModal()','primary','fa-plus'));
    }
    const countLine = `<p class="text-xs text-slate-400 mb-3 sm:hidden">${contacts.length} contact${contacts.length!==1?'s':''}</p>`;
  ...
  async function openAddModal(preselect = {}) {
    ...
    modal.open({
      ...
      footer: `
        ...
        <button onclick="contactsPage.submitCreate()" class="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">
  ...
  function openEditModal(id) {
    ...
    modal.open({
      ...
      footer: `
        <button onclick="modal.close()" class="px-4 py-2.5 text-sm font-medium text-slate-600 hover:text-slate-800">Cancel</button>
        <button onclick="contactsPage.submitUpdate(${id})" class="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">Save Changes</button>`,
    });
  }
  ...
  async function submitCreate() {
    ...
    try {
      ...
      if (typeof applicationDetailPage !== 'undefined' && data.application_id) applicationDetailPage.refreshDetail();
  ...
  function confirmDelete(id, name) {
    modal.confirm({ title: 'Delete Contact', message: `Delete <strong>${name}</strong>?`, onYes: () => deleteContact(id) });
  }
