...
const dashboardPage = (() => {
  ...
  function buildDashboard(s) {
    ...
    const statsRow = statCards.map(c => `
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 card-hover flex items-center gap-4">
        ...
        <div>
          <p class="text-3xl font-bold text-slate-800">${c.value}</p>
          <p class="text-slate-500 text-sm mt-0.5">${c.label}</p>
        </div>
      </div>
    `).join('');
    ...
    const metaCards = `
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 card-hover flex items-center gap-4">
        <div class="w-12 h-12 bg-violet-50 rounded-xl flex items-center justify-center flex-shrink-0">
          <i class="fa-solid fa-calendar-days text-violet-600 text-xl"></i>
        </div>
        <div>
          <p class="text-3xl font-bold text-slate-800">${s.this_month}</p>
          <p class="text-slate-500 text-sm mt-0.5">Applied This Month</p>
        </div>
      </div>
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 card-hover flex items-center gap-4">
        <div class="w-12 h-12 bg-sky-50 rounded-xl flex items-center justify-center flex-shrink-0">
          <i class="fa-solid fa-microphone text-sky-600 text-xl"></i>
        </div>
        <div>
          <p class="text-3xl font-bold text-slate-800">${s.total_interviews}</p>
          <p class="text-slate-500 text-sm mt-0.5">Total Interviews</p>
        </div>
      </div>
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 card-hover flex items-center gap-4">
        ...
        <div>
          <p class="text-3xl font-bold text-slate-800">${s.upcoming_interviews}</p>
          <p class="text-slate-500 text-sm mt-0.5">Upcoming Interviews</p>
        </div>
      </div>
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 card-hover flex items-center gap-4">
        ...
        <div>
          <p class="text-3xl font-bold text-slate-800">${s.avg_rating ? s.avg_rating + '/5' : '—'}</p>
          <p class="text-slate-500 text-sm mt-0.5">Avg. Self Rating</p>
        </div>
      </div>
    `;
    ...
    return `
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">${statsRow}</div>
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">${metaCards}</div>
